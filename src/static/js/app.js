...
async function populateUnitCheckboxes() {
  ...
  try {
    let response = await fetch(`${API_BASE_URL}/unit-names`);
    ...
    checkboxContainer.innerHTML =
      "<p class='text-red-500'>Üretim hatları yüklenirken bir hata oluştu.</p>";
  }
}
  ...
  timePeriodCheckboxes.forEach((checkbox) => {
    checkbox.addEventListener("change", function () {
      ...
      if (checkbox.checked) {
        ...
        timePeriodCheckboxes.forEach((cb) => {
          if (cb !== checkbox) cb.checked = false;
        });
      } else {
        // If unchecked, clear the date fields only if no other period is selected
        const anyChecked = Array.from(timePeriodCheckboxes).some(
          (cb) => cb.checked
        );
        if (!anyChecked) {
    ...
    .addEventListener("click", function () {
      ...
      if (!startDatetime || !endDatetime) {
        ...
        if (selectedPeriod) {
          ...
          const periodCheckbox = document.querySelector(
            `.time-period-checkbox[value="${selectedPeriod}"]`
          );
          if (periodCheckbox) {
